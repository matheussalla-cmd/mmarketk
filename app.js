...
function seedDemoData() {
  state.accounts = [
    ...
    { id: 'demo2', name: 'DEMO – Google Ads', platform: 'google', token: 'DEMO', accId: 'cid-000000', active: true, isDemo: true }
  ];
  state.campaigns = [
    ...
    { id: 'c4', name: 'Google Search – Marca', accountId: 'demo2', objective: 'trafego', status: 'ativa', budget: 120, spend: 3300, revenue: 19200, clicks: 5600, impressions: 48000, conversions: 210, platform: 'google', source: 'demo' },
    { id: 'c5', name: 'Google Search – Concorrentes', accountId: 'demo2', objective: 'trafego', status: 'pausada', budget: 90, spend: 1800, revenue: 5400, clicks: 2100, impressions: 22000, conversions: 48, platform: 'google', source: 'demo' },
...
async function syncAllAccounts() {
  ...
  for (const acc of active) {
    try {
      ...
      if (acc.platform === 'google')  total += await syncGoogleAccount(acc);
      if (acc.platform === 'tiktok')  total += await syncTikTokAccount(acc);
    } catch(e) {
...
async function syncGoogleAccount(account) {
  if (!account.token || !account.accId) {
    throw new Error('Token e Customer ID são obrigatórios para Google Ads');
  }
  showToast(`Sincronizando ${account.name} (Google Ads)...`, 'info');
  const customerId = account.accId.replace(/-/g, '');
  const { since, until } = getActiveDateRange();

  if (!account.developerToken) {
    throw new Error('Google Ads: Developer Token não configurado. Adicione-o ao editar a conta em Contas & Tokens.');
  }
  ...
  try {
    ...
    const res = await fetch(
      `${GOOGLE_ADS_BASE}/${GOOGLE_ADS_API_VERSION}/customers/${customerId}/googleAds:searchStream`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${account.token}`,
          'developer-token': account.developerToken,
    ...
    if (!res.ok) {
      ...
      try {
        ...
        msg = parsed[0]?.error?.message || parsed.error?.message || msg;
      } catch(e) {}
    ...
    (rows || []).forEach(batch => {
      (batch.results || []).forEach(row => {
        ...
        const m     = row.metrics;
        const spend = (parseInt(m.cost_micros || 0)) / 1e6;
        const obj = {
          id:          `google_${c.id}`,
          googleId:    c.id,
          name:        c.name,
          accountId:   account.id,
          objective:   mapGoogleObjective(c.advertising_channel_type),
        ...
        const idx = state.campaigns.findIndex(x => x.id === obj.id);
        if (idx >= 0) state.campaigns[idx] = obj; else state.campaigns.push(obj);
        synced++;
      });
    });
    return synced;
...
function mapGoogleObjective(channelType) {
  const m = {
    SEARCH: 'trafego', DISPLAY: 'awareness', SHOPPING: 'conversao',
    VIDEO: 'awareness', SMART: 'conversao', PERFORMANCE_MAX: 'conversao',
    DISCOVERY: 'awareness', DEMAND_GEN: 'conversao'
  };
  return m[channelType] || 'trafego';
}
...
//
//  COMO CONFIGURAR:
//  1. Acesse: ads.tiktok.com/marketing_api → My Apps → Criar app
//  2. Ative as permissões: Campaign Read, Report Read, Ad Read
//  3. Gere o Access Token via OAuth2:
//     URL: https://ads.tiktok.com/marketing_api/auth?app_id=XXX&redirect_uri=https://theusmarkt.netlify.app
//  4. Em Contas & Tokens:
//     - Token: Access Token gerado
//     - ID da Conta: Advertiser ID (encontrado em Conta > Configurações)
//
//  NOTA: TikTok Ads API exige App aprovado. Para testes use Sandbox.
// ═══════════════════════════════════════════════════════════════
...
async function syncTikTokAccount(account) {
  if (!account.token || !account.accId) {
    throw new Error('Token e Advertiser ID são obrigatórios para TikTok Ads');
  }
  showToast(`Sincronizando ${account.name} (TikTok Ads)...`, 'info');
  const { since, until } = getActiveDateRange();

  try {
    // 1. Busca campanhas
    const campRes = await fetch(
      `${TIKTOK_ADS_BASE}/campaign/get/?advertiser_id=${encodeURIComponent(account.accId)}&page_size=100`,
      { headers: { 'Access-Token': account.token, 'Content-Type': 'application/json' } }
    );
    if (!campRes.ok) throw new Error(`TikTok API HTTP ${campRes.status} – verifique o Access Token e Advertiser ID`);
    const campData = await campRes.json();
    if (campData.code !== 0) throw new Error(`TikTok: ${campData.message} (código ${campData.code})`);

    const campaigns = campData.data?.list || [];
    if (!campaigns.length) { showToast(`Nenhuma campanha em ${account.name}`, 'info'); return 0; }

    // 2. Relatório de métricas
    const reportParams = new URLSearchParams({
      advertiser_id: account.accId,
      report_type:   'BASIC',
      dimensions:    JSON.stringify(['campaign_id']),
      metrics:       JSON.stringify(['spend','impressions','clicks','ctr','cpc','reach','conversion','cost_per_conversion','frequency']),
      start_date:    since,
      end_date:      until,
      page_size:     '100'
    });
    ...
    const repRes = await fetch(
      `${TIKTOK_ADS_BASE}/report/integrated/get/?${reportParams}`,
      { headers: { 'Access-Token': account.token } }
    );
    const repData = repRes.ok ? await repRes.json() : { data: { list: [] } };
    const metricsMap = {};
    (repData.data?.list || []).forEach(row => {
    ...
    campaigns.forEach(camp => {
      ...
      const obj = {
        id:          `tiktok_${camp.campaign_id}`,
        tiktokId:    camp.campaign_id,
        name:        camp.campaign_name,
        accountId:   account.id,
        objective:   camp.objective_type?.toLowerCase().includes('conversion') ? 'conversao' :
                     camp.objective_type?.toLowerCase().includes('lead') ? 'leads' : 'trafego',
        status:      camp.operation_status === 'ENABLE' ? 'ativa' : 'pausada',
...
function renderCharts(camps) {
  ...
  if(c2) state.charts.channels=new Chart(c2,{type:'doughnut',
    data:{labels:Object.keys(pt).map(p=>({meta:'Meta Ads',google:'Google Ads',tiktok:'TikTok Ads',linkedin:'LinkedIn Ads'}[p]||p)),
...
function renderFunnel(camps) {
  ...
  if (label) label.textContent = campPlatformFilter === 'all' ? 'Geral' : ({meta:'Meta',google:'Google',tiktok:'TikTok',linkedin:'LinkedIn'}[campPlatformFilter]||campPlatformFilter);
...
function renderCplRoasPlat(camps) {
  ...
  const gc = isLight ? 'rgba(0,0,0,0.06)' : 'rgba(255,255,255,0.05)';
  const tc = isLight ? '#6b7280' : '#8892a4';
  const platColors = { meta:'#4a9eff', google:'#f0c040', tiktok:'#f472b6', linkedin:'#818cf8' };
  const platNames  = { meta:'Meta', google:'Google', tiktok:'TikTok', linkedin:'LinkedIn' };
...
function renderActiveCampsTable(camps) {
  ...
  const platIcon = {
    ...
    google:  `<div class="plat-icon-cell google">🔴</div>`,
    tiktok:  `<div class="plat-icon-cell tiktok">🎵</div>`,
    linkedin:`<div class="plat-icon-cell linkedin">🔵</div>`
  };
...
function renderAccountsList(){
  ...
  const icons={meta:'📘',google:'🔴',tiktok:'🎵',linkedin:'🔵'};
  const names={meta:'Meta Ads',google:'Google Ads',tiktok:'TikTok Ads',linkedin:'LinkedIn Ads'};
    ...
    html+=realAccounts.map((a)=>{
      ...
      return `<div class="acc-card" id="acc-card-${a.id}">
        ...
        <div class="acc-info">
          ...
          ${a.platform==='google' ? `<div style="font-size:11px;margin-top:4px;padding:4px 8px;border-radius:6px;display:inline-block;${a.developerToken ? 'background:rgba(62,207,142,.1);color:var(--green)' : 'background:rgba(248,113,113,.1);color:var(--red)'}">
            ${a.developerToken ? '✅ Developer Token configurado' : '⚠️ Developer Token ausente — clique em Editar'}
          </div>` : ''}
          ${a.platform==='tiktok' ? `<div style="font-size:11px;margin-top:4px;color:var(--text2)">🎵 Advertiser ID · Access Token</div>` : ''}
          ${a.lastSync
...
async function syncAccountNow(id){
  ...
  try {
    ...
    else if (acc.platform === 'google') n = await syncGoogleAccount(acc);
    else if (acc.platform === 'tiktok') n = await syncTikTokAccount(acc);
...
function showSyncError(accId, msg, platform) {
  ...
  const tips = {
    ...
    google: `
      • Você configurou o <strong>Developer Token</strong>? Obtenha em <a href="https://ads.google.com" target="_blank">ads.google.com</a> → Ferramentas → Central da API<br>
      • Clique em <strong>✎ Editar</strong> na conta e preencha o campo "Developer Token (Google Ads)"<br>
      • O <strong>Access Token OAuth2</strong> está válido e não expirado?<br>
        Gere um novo em <a href="https://console.cloud.google.com" target="_blank">console.cloud.google.com</a> → Credenciais → OAuth 2.0<br>
      • O <strong>Customer ID</strong> está correto? Use apenas números, sem hífens: <code>1234567890</code><br>
      • A <strong>Google Ads API</strong> está ativada no seu projeto do Google Cloud?<br>
      • O token OAuth2 tem o escopo <code>https://www.googleapis.com/auth/adwords</code>?`,

    tiktok: `
      • O <strong>Access Token</strong> está válido? Gere um novo em <a href="https://ads.tiktok.com/marketing_api" target="_blank">TikTok Marketing API</a><br>
      • O <strong>Advertiser ID</strong> está correto? Encontre em TikTok Ads Manager → Conta → Configurações<br>
      • O App TikTok tem as permissões: <code>Campaign Read</code>, <code>Report Read</code>, <code>Ad Read</code>?<br>
      • Para testes sem aprovação do App, use o ambiente <strong>Sandbox</strong>`,

    linkedin: `
      • O <strong>Access Token</strong> está válido e com a permissão <code>r_ads</code>?<br>
      • O <strong>ID da Conta</strong> (Account ID) está correto? Encontre no Campaign Manager<br>
      • O token OAuth2 não está expirado? Tokens LinkedIn expiram em 60 dias`,
  };

  const platNames  = { meta:'Meta Ads', google:'Google Ads', tiktok:'TikTok Ads', linkedin:'LinkedIn Ads' };
  const platColors = { meta:'#4a9eff', google:'#ea4335', tiktok:'#ff2d55', linkedin:'#0a66c2' };
  const platIcons  = { meta:'📘', google:'🔴', tiktok:'🎵', linkedin:'🔵' };

  const box = document.createElement('div');
  box.id = `err-${accId}`;
  box.className = 'meta-error-box';
  box.style.borderColor = (platColors[platform] || '#f87171') + '55';
  box.innerHTML = `
    ...
    <div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap">
      ...
      ${platform==='google' ? `<button class="btn-sm" style="border-color:var(--blue);color:var(--blue)" onclick="openEditAccount('${accId}')">✎ Editar e adicionar Developer Token</button>` : ''}
    </div>`;
...
function openEditAccount(accId) {
  ...
  if (acc.developerToken) {
    const devField = document.getElementById('mDevToken');
    if (devField) devField.value = acc.developerToken;
  }
  onPlatformChange();
...
function updateAccount(accId) {
  ...
  state.accounts[idx] = {
    ...
    name, token, platform,
    accId: accIdVal,
    ...(platform === 'google' && devToken ? { developerToken: devToken } : {})
  };
  saveState();
...
function onPlatformChange() {
  ...
  const devWrap = document.getElementById('devTokenWrap');
  const accLabel= document.getElementById('mAccIdLabel');
  const accInput= document.getElementById('mAccId');

  const hints = {
    meta: `<b>Meta Ads:</b> Token via <a href="https://developers.facebook.com/tools/explorer" target="_blank">Graph API Explorer</a> com permissões <code>ads_read</code>, <code>ads_management</code>, <code>business_management</code>. ID no formato <code>act_XXXXXXXXXX</code>.`,
    google: `<b>Google Ads:</b> Access Token via OAuth2 (<a href="https://console.cloud.google.com" target="_blank">console.cloud.google.com</a>). Developer Token em <a href="https://ads.google.com" target="_blank">ads.google.com</a> → Ferramentas → Central da API. Customer ID sem hífens: <code>1234567890</code>.`,
    tiktok: `<b>TikTok Ads:</b> Access Token via <a href="https://ads.tiktok.com/marketing_api" target="_blank">TikTok Marketing API</a> → Meus Apps. Advertiser ID em Conta → Configurações.`,
    linkedin: `<b>LinkedIn Ads:</b> Access Token via <a href="https://www.linkedin.com/developers" target="_blank">LinkedIn Developers</a> com permissão <code>r_ads</code>. ID da conta em Campaign Manager.`
  };
  if (hint) hint.innerHTML = hints[p] || '';
  if (devWrap) devWrap.style.display = p === 'google' ? 'block' : 'none';
  if (accLabel) accLabel.textContent = p === 'tiktok' ? 'Advertiser ID' : p === 'google' ? 'Customer ID' : 'ID da Conta';
  if (accInput) accInput.placeholder = p === 'meta' ? 'act_123456789' : p === 'google' ? '1234567890' : p === 'tiktok' ? '123456789' : 'ID da conta';
}
